 ...
 * @ClassName leetcode496 Next Greater Element I
...
public class Solution {
    private HashMap<Integer, Integer> mapNumsLocation;

    public int[] nextGreaterElement(int[] findNums, int[] nums) {
        int[] result = new int[findNums.length];
        mapNumsLocation = new HashMap<>();
        for (int i = 0; i < nums.length; i++) {
            mapNumsLocation.put(nums[i], i);//将nums数组中的数字与其所在位置记录下来.
        }
        for (int i = 0; i < findNums.length; i++) {
            int findNum = findNums[i];
            boolean flag = true;
            for (int j = mapNumsLocation.get(findNum) + 1; j < nums.length; j++) {
                if (findNum < nums[j]) {
                    result[i] = nums[j];
                    flag = false;
                    break;
